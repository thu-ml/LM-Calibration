...
do
  ...
  accelerate launch run_mc_k.py \
    ...
    --reinit \
    --k $k \
    --output_dir ./outputs/ckpts/$TASK_NAME/${MODEL_NAME}_reinit_k=${k}_seed=$SEED \
    --conf_dir ./outputs/conf/$TASK_NAME/${EVAL_SPLIT}/${MODEL_NAME}_reinit_k=${k}_seed=$SEED

  # OOD Eval
  python run_mc_k.py \
    --model_name_or_path ./outputs/ckpts/$TASK_NAME/${MODEL_NAME}_reinit_k=${k}_seed=$SEED \
    --dataset_name $OOD_TASK \
    --max_length 256 \
    --per_device_train_batch_size 32 \
    --eval_split ${EVAL_SPLIT} \
    --conf_dir ./outputs/conf/$OOD_TASK/${EVAL_SPLIT}/${MODEL_NAME}_reinit_k=${k}_seed=$SEED
done
done
done
