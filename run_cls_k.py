...
def main():
    ...
    if args.do_train and args.k != 0:
        # ReInit-K
        if args.reinit:
            model_sketch = AutoModelForSequenceClassification.from_config(config)
            for l1, l2 in zip(model.roberta.encoder.layer[-args.k:], model_sketch.roberta.encoder.layer[-args.k:]):
                for (n1, p1), (n2, p2) in zip(l1.named_parameters(), l2.named_parameters()):
                    assert n1 == n2
                    with torch.no_grad():
                        p1.fill_(0).add_(p2)
        # Top-K
        if args.top:
            ...
            for i in range(config.num_hidden_layers - args.k):
                for (n, p) in model.roberta.encoder.layer[i].named_parameters():
                    p.requires_grad = False
