...
from models import Variational, Generator, VAE, DataCollatorForMultiTask, LabelSmoothingLoss
...
def parse_args():
    ...
    parser.add_argument("--temperature", type=float, default=1, help="Temperature of MLM task")
    ...
    parser.add_argument("--mlm_len", type=int, default=32)
    parser.add_argument("--mlm_batch_size", type=int, default=32)
    parser.add_argument("--log_epoch", action="store_true")
    parser.add_argument("--pplm_iter", type=int, default=3)
...
def main():
    ...
    if hasattr(model_pretrained, "roberta"):
        variational = Variational(copy.deepcopy(model_pretrained.roberta))
    elif hasattr(model_pretrained, "bert"):
        variational = Variational(copy.deepcopy(model_pretrained.bert))

    if hasattr(model_pretrained, "lm_head"):
        generator = Generator(copy.deepcopy(model_pretrained.lm_head), config)
    elif hasattr(model_pretrained, "cls"):
        generator = Generator(copy.deepcopy(model_pretrained.cls), config)
