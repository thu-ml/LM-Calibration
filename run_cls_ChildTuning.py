...
""" Finetuning a 🤗 Transformers model for sequence classification on GLUE.
    Modified from https://github.com/alibaba/AliceMind/tree/main/ChildTuning
"""
import argparse
from ast import arg
import logging
...
from models.child_tuning import ChildTuningAdamW
...
def parse_args():
    ...
    # ChildTuning
    parser.add_argument("--mode", type=str, default="ChildTuning-F", choices=["ChildTuning-F", "ChildTuning-D"])
    parser.add_argument("--reserve_p", type=float, default=0.0)
...
def main():
    ...
    optimizer = ChildTuningAdamW(optimizer_grouped_parameters, lr=args.learning_rate, mode=args.mode, reserve_p=args.reserve_p)
    ...
    if args.mode == "ChildTuning-D":
        ...
        model.train()
        progress_bar = tqdm(range(N), disable=not accelerator.is_local_main_process)
        ...
        logger.info(f"Polar => {polar}")
        accelerator.wait_for_everyone()
        optimizer.optimizer.set_gradient_mask(gradient_mask) # May be there is a better way to do that...
