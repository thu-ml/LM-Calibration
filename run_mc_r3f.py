...
def parse_args():
    ...
    # R3F
    parser.add_argument("--noise_type", type=str, choices=["normal", "uniform"], default="uniform")
    parser.add_argument("--r3f_lambda", type=float, default=1.)
    parser.add_argument("--r3f_eps", type=float, default=1.)
...
def main():
    ...
    if args.noise_type in {"normal"}:
        noise_sampler = torch.distributions.normal.Normal(
            loc=0.0, scale=args.r3f_eps
        )
    elif args.noise_type in {"uniform"}:
        noise_sampler = torch.distributions.uniform.Uniform(
            low=-args.r3f_eps, high=args.r3f_eps
        )
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                input_embeds = model.roberta.embeddings.word_embeddings(flat_input_ids)
                input_embeds = input_embeds.view(input_ids.size(0), input_ids.size(1), input_ids.size(2), -1)

                input_outputs = model(**batch, inputs_embeds=input_embeds)
                input_logits = input_outputs.logits
                
                noise = noise_sampler.sample(sample_shape=input_embeds.shape).to(input_embeds)
                noised_embeds = input_embeds.detach().clone() + noise
                noised_outputs = model(**batch, inputs_embeds=noised_embeds)
                ...
                loss = input_outputs.loss + args.r3f_lambda * symm_kl
