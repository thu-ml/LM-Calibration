# coding=utf-8
# Official implementation for: Recall and Learn: Fine-tuning Deep Pretrained Language Models with Less Forgetting (Accepted by EMNLP 2020).
...
"""RecAdam optimizer
   Modified version, only constant mode supported."""
        ...
        for group in self.param_groups:
            for p, pp in zip(group["params"], group["pretrain_params"]):
                ...
                if group['anneal_w'] > 0.0:
                    ...
                    p.data.add_(-group["lr"] * group["anneal_w"], p.data - pp.data)
                else:
                    p.data.addcdiv_(-step_size, exp_avg, denom)
